...
\section*{How RNA folding is predicted}
\label{sec-1}
...
\begin{itemize}
\item a specification of a state of RNA $s$,
\item an energy function, which takes a state and outputs a physical
  energy,
\item the partition function, the sum of the Boltmann factors over every
  possible state $Z = \sum_{s'} e^{-E(s)/kT}$.
\end{itemize}
\section*{States of RNA}
\label{sec-2}
...
as a list of base pairs $(i,j)$. For \textbf{tractability} reasons, in
...
\section*{Sampling RNA Structures}
\label{sec-5}
